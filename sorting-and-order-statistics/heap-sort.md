...
* maxHeapify 时间复杂度为 O(lgn)，它是维护最大堆性质的关键。

* buildMaxHeap 具有线性时间复杂度，功能是从无序的输入数组中构造一个最大堆。

* heapSort 时间复杂度为 O(n lgn)，功能是对一个数组进行原址排序。

* insert、extractMax、increaseKey 和 maximum 时间复杂度为 O(lgn)，功能是利用堆实现一个优先队列。
