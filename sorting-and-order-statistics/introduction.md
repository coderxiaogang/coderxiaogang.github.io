...
在某种意义上，正是这些实现的细节将一个算法与成熟的程序区分开来。一个排序算法描述排序的方法（method），而不管我们是在排序单独的数还是包含很多卫星数据的记录。因此，当关注排序问题时，我们通常假定输入只是由数字组成。将一个对数字进行排序的算法转换为一个对记录进行排序的程序，在概念上是很直接的，当然在具体的工程情境下，其它一些细节问题可能会使实际的编程工作遇到很多挑战。
...
插入排序最坏情况下可以在O(n^2)时间内将n个数排好序。但是，由于其内层循环非常紧凑，对于小规模输入，插入排序是一种非常快的原址排序算法（如果输入数组中仅有常数个元素需要在排序过程中存储在数组外，则称算法是原址（in place）的）。

归并排序有更好的渐近运行时间 O(n*lgn)，但它所使用的 merge 过程并不是原址的。

堆排序是一种 O(n*lgn) 时间的原址排序算法，它使用了一种被称为堆的重要数据结构，堆还可以用来实现优先队列。

快速排序也是一种原址排序算法，但最坏情况运行时间为 O(n^2)。然而它的期望运行时间为 Θ(n*lgn)，而且在实际应用中通常比堆排序快。与插入排序类似，快速排序的代码也很紧凑，因此运行时间中隐含的常数系数很小。快速排序是排序大数组的最常用算法。
