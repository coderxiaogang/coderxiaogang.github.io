...
insert 的运行时间怎样呢？由于一棵有 n 个结点的红黑树的高度 h 为 O(lgn)，因此，首先将 z 按普通二叉搜索树的方式插入到树中需要花费 O(h) = O(lgn) 时间。在 insertFixUp 中，仅当情况 1 发生，然后指针 z 沿着树上升两层，while 循环才回重复执行，所以 while 循环可能被执行的总次数为 O(lgn)。因此，insert 总共花费 O(lgn) 时间。此外，该程序所做的旋转从不超过 2 次，因为只要执行了情况 2 或情况 3，while 循环就结束了。
