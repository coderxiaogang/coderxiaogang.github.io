...
* union 将包含 x 和 y 的两个动态集合（表示为 S[x] 和 S[y]）合并成一个新的集合，即这两个集合的并集（假定在操作之前这两个集合是不相交的）。虽然 union 的很多实现中特别地选择 S[x] 或 S[y] 的代表作为新的代表，然而结果集的代表可以是 S[x] ∪ S[y] 中的任何成员。由于我们要求各个集合不相交，故要消除原有的集合 S[x] 和 S[y]，即把它们从 S 中删除。实际上，我们经常把其中一个集合的元素并入另一个集合中，来代替删除操作。
